...
${APP} con metadata --from-file ${TMP_PATH}/options.json > ${TMP_PATH}/meta.json
if [ $? != 0 ]; then
...
${APP} con parse --from-file ${TMP_PATH}/unsigned_tx.json > ${TMP_PATH}/parse_unsigned.json
if [ $? != 0 ]; then 
...
${APP} con parse --from-file ${TMP_PATH}/signed_tx.json > ${TMP_PATH}/parse_signed.json
if [ $? != 0 ]; then
...
# /construction/hash
${APP} con hash --from-file ${TMP_PATH}/signed_tx.json > ${TMP_PATH}/hash.json
if [ $? != 0 ]; then
...
# /construction/submit
${APP} con submit --from-file ${TMP_PATH}/signed_tx.json > ${TMP_PATH}/submit.json
if [ $? != 0 ]; then
